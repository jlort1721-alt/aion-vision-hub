...
import { registerIsapiIngestRoutes } from "./modules/isapi-ingest/routes.js";
import { registerStreamsRoutes } from "./modules/streams/index.js";
...
import { registerHikBridgeRoutes } from "./modules/hik-bridge/routes.js";
...
import {
  ...
  registerTwilioWebhookRoutes,
} from "./modules/twilio/routes.js";
...
export async function buildApp() {
  ...
  await app.register(swagger, {
    openapi: {
      info: {
        ...
        description: "Enterprise VMS and IoT Core Engine",
        version: "1.0.0",
      },
      components: {
        securitySchemes: {
  ...
  try {
    ...
    /* @fastify/compress not installed */
  }
  ...
  await app.register(registerOperationalDataRoutes, {
    prefix: "/operational-data",
  });
  await app.register(registerVisitorPreregistrationRoutes, {
    prefix: "/pre-registrations",
  });
  ...
  await app.register(registerIsapiIngestRoutes, { prefix: "/isapi" });
  await app.register(registerStreamsRoutes, { prefix: "/streams" });
  ...
  await app.register(registerNotificationTemplateRoutes, {
  ...
  await app.register(registerScheduledReportRoutes, {
  ...
  await app.register(registerDeviceControlRoutes, {
  ...
  await app.register(registerInternalAgentRoutes, {
  ...
  // Dahua REGISTRO — Direct device control + Active Registration monitor
  await app.register(registerDahuaRoutes, { prefix: "/dahua" });

  // Face Recognition Framework (CodeProject.AI / DeepStack / Frigate)
  await app.register(registerFaceRecognitionRoutes, {
    prefix: "/face-recognition",
  });

  // Heat Mapping — Traffic analysis and zone activity visualization
  await app.register(registerHeatMappingRoutes, {
    prefix: "/analytics/heatmap",
  });
  ...
  await app.register(registerOperatorAssignmentRoutes, {
    prefix: "/operator-assignments",
  });
  ...
  await app.register(registerCameraDetectionRoutes, {
    prefix: "/camera-detections",
  });
  ...
  // Hikvision HCNetSDK bridge (PTZ, alarms, recordings, discovery via Python sidecar)
  await app.register(registerHikBridgeRoutes, { prefix: "/hik-bridge" });
  ...
  await app.register(registerTwilioWebhookRoutes, {
  ...
  const { registerVPSPlugins, registerWallSystem } =
    await import("./modules/vps-plugins/index.js");
  ...
  // NOTE: Will be deprecated once REGISTRO is stable for all 12 Dahua devices
  imouStreamManager.start();

  // Start Dahua REGISTRO monitor — polls go2rtc for dvrip-connected devices
  // and auto-registers their channels. Set DAHUA_MODE=registro to enable (default)
  dahuaRegistroMonitor.start();

  // Start Dahua event polling — detects motion, intrusion, tamper, etc. from Dahua devices
  dahuaEvents.start();
  ...
  app.addHook("onClose", async () => {
    ...
    dahuaRegistroMonitor.stop();
    dahuaEvents.stopPolling();
