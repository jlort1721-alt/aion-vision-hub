...
export class OperationalDataService {
  ...
  async residentsList(tenantId: string, filters: ListFilters) {
    ...
    const col = sanitizeSort(sort_by ?? "full_name", [
      "full_name",
      "unit_number",
      "created_at",
      "updated_at",
    ...
    if (search)
      conditions.push(
        sql`(full_name ILIKE ${"%" + search + "%"} OR unit_number ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM residents WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM residents WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);

    const total =
      ((countResult as unknown as Record<string, unknown>[])[0]
        ?.total as number) ?? 0;
  ...
  async residentsStats(tenantId: string) {
    ...
    `);
    const totals = (
      totalsResult as unknown as Record<string, unknown>[]
  ...
  async residentsGroupedByModule(tenantId: string) {
    ...
    for (const row of rows) {
      const siteName =
        ((row as Record<string, unknown>).site_name as string) ?? "Sin sitio";
      const unitNumber =
        ((row as Record<string, unknown>).unit_number as string) ?? "";
      ...
      if (!grouped[siteName][module][apartment])
        grouped[siteName][module][apartment] = [];
      grouped[siteName][module][apartment].push(row);
    }
    return grouped;
  }

  async residentsBulkImport(
    tenantId: string,
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!fullName || !unitNumber) {
        errors.push({
          row: i + 1,
          reason: "full_name y unit_number son obligatorios",
        });
        skipped++;
        continue;
      }
  ...
  async vehiclesList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "plate", [
      "plate",
      "brand",
      "model",
      "color",
    ...
    if (search)
      conditions.push(
        sql`(plate ILIKE ${"%" + search + "%"} OR brand ILIKE ${"%" + search + "%"} OR model ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM vehicles WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM vehicles WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
        ...
        if (key === "plate") {
          entries.push({
            ...
            value:
              data[key] === null
                ? null
                : String(data[key]).toUpperCase().trim(),
          });
        } else {
          entries.push({
            key,
  ...
  async biometricRecordsList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "created_at", [
      "created_at",
      "resident_name",
      "biometric_type",
      "site_id",
    ...
    if (site_id) conditions.push(sql`site_id = ${site_id}`);
    if (search)
      conditions.push(sql`(resident_name ILIKE ${"%" + search + "%"})`);
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM biometric_records WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM biometric_records WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async biometricRecordsCreate(
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `biometric_records.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }

  async biometricRecordsUpdate(
    id: string,
    tenantId: string,
    data: Record<string, unknown>,
  ) {
  ...
  async consignasList(
    tenantId: string,
    filters: ListFilters & { unit_number?: string },
  ) {
    const {
      page = 1,
      limit = 50,
    ...
    if (search)
      conditions.push(
        sql`(description ILIKE ${"%" + search + "%"} OR unit_number ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM consignas WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM consignas WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async siteAdministratorsList(tenantId: string, filters: ListFilters) {
    ...
    if (search)
      conditions.push(
        sql`(full_name ILIKE ${"%" + search + "%"} OR email ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM site_administrators WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM site_administrators WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async siteAdministratorsCreate(
    tenantId: string,
    data: Record<string, unknown>,
  ) {
    const result = await db.execute(sql`
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `site_administrators.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }

  async siteAdministratorsUpdate(
    id: string,
    tenantId: string,
    data: Record<string, unknown>,
  ) {
  ...
  async sirenTestsList(
    tenantId: string,
    filters: DateRangeFilters & { result?: string },
  ) {
    const {
      page = 1,
      limit = 50,
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "tested_at", [
      "tested_at",
      "site_id",
      "result",
      "created_at",
    ...
    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM siren_tests WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM siren_tests WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async equipmentRestartsList(tenantId: string, filters: DateRangeFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "restarted_at", [
      "restarted_at",
      "site_id",
      "equipment_name",
      "created_at",
    ...
    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM equipment_restarts WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM equipment_restarts WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async equipmentRestartsCreate(
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `equipment_restarts.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }
  ...
  async siteDoorInventoryList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "door_name", [
      "door_name",
      "door_type",
      "floor",
      "created_at",
    ...
    if (search)
      conditions.push(
        sql`(door_name ILIKE ${"%" + search + "%"} OR location_description ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM site_door_inventory WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM site_door_inventory WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async siteDoorInventoryCreate(
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `site_door_inventory.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }

  async siteDoorInventoryUpdate(
    id: string,
    tenantId: string,
    data: Record<string, unknown>,
  ) {
  ...
  async elevatorInfoList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "elevator_name", [
      "elevator_name",
      "brand",
      "last_maintenance",
      "created_at",
    ...
    if (search)
      conditions.push(
        sql`(elevator_name ILIKE ${"%" + search + "%"} OR brand ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM elevator_info WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM elevator_info WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async guardSchedulesList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "guard_name", [
      "guard_name",
      "shift_start",
      "shift_end",
      "day_of_week",
    ...
    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM guard_schedules WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM guard_schedules WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async monitoringCredentialsList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "service_name", [
      "service_name",
      "platform",
      "created_at",
      "site_id",
    ...
    if (search)
      conditions.push(
        sql`(service_name ILIKE ${"%" + search + "%"} OR platform ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM monitoring_credentials WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT id, tenant_id, site_id, service_name, platform, username, url, port, notes, status, created_at, updated_at FROM monitoring_credentials WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async monitoringCredentialsCreate(
    tenantId: string,
    data: Record<string, unknown>,
  ) {
    const result = await db.execute(sql`
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `monitoring_credentials.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }

  async monitoringCredentialsUpdate(
    id: string,
    tenantId: string,
    data: Record<string, unknown>,
  ) {
  ...
  async operatorTrainingsList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "training_date", [
      "training_date",
      "operator_name",
      "topic",
      "created_at",
    ...
    if (search)
      conditions.push(
        sql`(operator_name ILIKE ${"%" + search + "%"} OR topic ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM operator_trainings WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM operator_trainings WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async operatorTrainingsCreate(
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `operator_trainings.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }
  ...
  async siteCctvDescriptionList(tenantId: string, filters: ListFilters) {
    ...
    const offset = calcOffset(page, limit);
    const col = sanitizeSort(sort_by ?? "camera_name", [
      "camera_name",
      "location",
      "camera_type",
      "created_at",
    ...
    if (search)
      conditions.push(
        sql`(camera_name ILIKE ${"%" + search + "%"} OR location ILIKE ${"%" + search + "%"})`,
      );
    const where = sqlAnd(conditions);

    const [countResult, dataResult] = await Promise.all([
      db.execute(
        sql`SELECT count(*)::int AS total FROM site_cctv_description WHERE ${where}`,
      ),
      db.execute(
        sql`SELECT * FROM site_cctv_description WHERE ${where} ORDER BY ${sql.raw(col)} ${sql.raw(ord)} LIMIT ${limit} OFFSET ${offset}`,
      ),
    ]);
  ...
  async siteCctvDescriptionCreate(
    tenantId: string,
    data: Record<string, unknown>,
  ) {
    const result = await db.execute(sql`
    ...
    const rows = result as unknown as Record<string, unknown>[];
    logger.info(
      `site_cctv_description.create tenant=${tenantId} id=${rows[0]?.id}`,
    );
    return rows[0];
  }

  async siteCctvDescriptionUpdate(
    id: string,
    tenantId: string,
    data: Record<string, unknown>,
  ) {
  ...
  async overallStats(tenantId: string) {
    const result = await db.execute(sql`
      SELECT
        ...
        (SELECT count(*)::int FROM siren_tests WHERE site_id IN (SELECT id FROM sites WHERE tenant_id = ${tenantId})) AS siren_tests_count,
        (SELECT count(*)::int FROM equipment_restarts) AS equipment_restarts_count,
        (SELECT count(*)::int FROM site_door_inventory WHERE site_id IN (SELECT id FROM sites WHERE tenant_id = ${tenantId})) AS door_inventory_count,
        (SELECT count(*)::int FROM elevator_info WHERE site_id IN (SELECT id FROM sites WHERE tenant_id = ${tenantId})) AS elevator_info_count,
        (SELECT count(*)::int FROM guard_schedules WHERE site_id IN (SELECT id FROM sites WHERE tenant_id = ${tenantId})) AS guard_schedules_count,
        (SELECT count(*)::int FROM monitoring_credentials) AS monitoring_credentials_count,
        (SELECT count(*)::int FROM operator_trainings) AS operator_trainings_count,
        (SELECT count(*)::int FROM site_cctv_description WHERE site_id IN (SELECT id FROM sites WHERE tenant_id = ${tenantId})) AS cctv_descriptions_count
    `);
    const rows = result as unknown as Record<string, unknown>[];
    return rows[0] ?? {};
  }
