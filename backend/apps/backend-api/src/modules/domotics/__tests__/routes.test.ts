...
describe('Domotics Routes', () => {
  ...
  describe('GET /domotics/:id', () => {
    it('returns 200 with device details', async () => {
      const res = await app.inject({ method: 'GET', url: '/domotics/dev-001' });
      expect(res.statusCode).toBe(200);
      const body = res.json();
      expect(body.data).toHaveProperty('id', 'dev-001');
  ...
  describe('POST /domotics', () => {
    it('creates device with valid payload', async () => {
      const res = await app.inject({
        method: 'POST',
        url: '/domotics',
        payload: { name: 'New Relay', type: 'relay', brand: 'Sonoff', model: 'BASIC R3' },
      });
    ...
    it('accepts valid device types', async () => {
      for (const type of ['relay', 'sensor', 'switch', 'lock', 'siren', 'light', 'door']) {
        const res = await app.inject({
          method: 'POST',
          url: '/domotics',
