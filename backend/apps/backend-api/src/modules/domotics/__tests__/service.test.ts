...
describe('DomoticService', () => {
  ...
  describe('getById()', () => {
    ...
    it('throws when device not found', async () => {
      ...
      ).rejects.toThrow('Domotic device nonexistent not found');
    });
  });
  ...
  describe('create()', () => {
    it('creates device and returns it', async () => {
      ...
      const result = await domoticService.create(
        { name: 'New Relay', type: 'relay', brand: 'Sonoff', model: 'BASIC R3' },
        tenantId
      );
  ...
  describe('update()', () => {
    it('updates device and returns it', async () => {
      ...
      const result = await domoticService.update('dev-1', { name: 'Updated Name' }, tenantId);
  ...
  describe('executeAction()', () => {
    it('toggles state from off to on', async () => {
      ...
      mockResult.value = [device];
      mockInsert.mockResolvedValue([{ id: 'act-1', action: 'toggle', result: 'on' }]);

      const result = await domoticService.executeAction('dev-1', 'toggle', 'user-1', tenantId);
    ...
    it('toggles state from on to off', async () => {
      ...
      mockResult.value = [device];
      mockInsert.mockResolvedValue([{ id: 'act-2', action: 'toggle', result: 'off' }]);
    ...
    it('throws when device not found for action', async () => {
      ...
      await expect(
        domoticService.executeAction('nonexistent', 'toggle', 'user-1', tenantId)
      ).rejects.toThrow();
    });
  });
