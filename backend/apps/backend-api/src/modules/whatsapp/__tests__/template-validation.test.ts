...
vi.mock("drizzle-orm", () => ({
  eq: vi
    .fn()
    .mockImplementation((a: unknown, b: unknown) => ({ op: "eq", a, b })),
  and: vi
    .fn()
    .mockImplementation((...args: unknown[]) => ({ op: "and", args })),
  desc: vi.fn(),
  lt: vi.fn(),
}));
...
vi.mock("../provider.js", () => ({
  MetaCloudAPIProvider: vi.fn().mockImplementation(function () {
    return {
