  ...
  app.get('/detailed', async (_request, reply) => {
    ...
    const checkService = async (
      ...
      try {
        ...
        checks[name] = { status: 'unhealthy', latency_ms: Date.now() - t0, detail: (err as Error).message };
        allHealthy = false;
      }
    };

    const checkTcpPort = (host: string, port: number, timeoutMs = 3000): Promise<void> =>
    ...
    await Promise.allSettled([
      checkService('postgresql', async () => { await db.execute(sql`SELECT 1`); }),
      ...
      checkService('go2rtc', async () => {
        ...
        const res = await fetch(`${GO2RTC}/api/streams`, { signal: AbortSignal.timeout(5000) });
      ...
      checkService('mqtt', async () => { await checkTcpPort('localhost', 1883); }),

      checkService('asterisk', async () => { await checkTcpPort('localhost', 5038); }),

      checkService('face_recognition', async () => {
        const FACE_URL = process.env.FACE_RECOGNITION_URL || 'http://localhost:5050';
        const res = await fetch(`${FACE_URL}/health`, { signal: AbortSignal.timeout(5000) });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
      }),
    ]);
    ...
    try {
      ...
      checks.memory = { status: heapPct > 90 ? 'warning' : 'healthy', detail: `heap ${heapPct}% (${Math.round(mem.rss / 1048576)}MB RSS)` };
    } catch { /* skip */ }
