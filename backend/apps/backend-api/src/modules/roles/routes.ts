...
export async function registerRoleRoutes(app: FastifyInstance) {
  ...
  app.post('/assign', { preHandler: [requireRole('super_admin', 'tenant_admin')] }, async (request, reply) => {
    const body = assignRoleSchema.parse(request.body);
    const data = await service.assign(body, request.tenantId, request.userRole);
