...
vi.mock("../orchestration-service.js", () => ({
  orchestrationService: {
    initiateOutbound: vi
      .fn()
      .mockResolvedValue({ sessionId: "sess-1", status: "ringing" }),
    handleInbound: vi
      .fn()
      .mockResolvedValue({ sessionId: "sess-2", status: "answered" }),
    updateSession: vi
      .fn()
      .mockResolvedValue({ sessionId: "sess-1", status: "active" }),
    endCall: vi
      .fn()
      .mockResolvedValue({ sessionId: "sess-1", status: "ended" }),
    ...
    getCallStats: vi
      .fn()
      .mockResolvedValue({ total: 10, avgDuration: 60, byDirection: {} }),
  },
}));
  ...
  describe("POST /intercom/calls", () => {
    it("creates call log entry", async () => {
      const res = await app.inject({
        ...
        url: "/intercom/calls",
        payload: {
          deviceId: "00000000-0000-4000-a000-000000000001",
          direction: "inbound",
          durationSeconds: 30,
          attendedBy: "ai",
          status: "completed",
          sectionId: "00000000-0000-4000-a000-000000000010",
        },
      });
      expect(res.statusCode).toBe(201);
    });
