...
function makeChainable(): any {
  const obj: any = {};
  obj.limit = mockLimit;
  obj.orderBy = vi.fn<any>((..._args: any[]) => makeChainable());
  obj.returning = mockReturning;
  obj.offset = mockOffset;
  obj.groupBy = vi.fn<any>(() => makeChainable());
  obj.where = vi.fn<any>(() => makeChainable());
  // Make it thenable so `const [r] = await db.select().from().where()` works
...
const mockOrderBy = vi.fn<any>(() => makeChainable());
const mockWhere = vi.fn<any>(() => makeChainable());
...
describe("AlertService", () => {
  ...
  beforeEach(() => {
    ...
    mockOrderBy.mockImplementation(() => makeChainable());
  ...
  describe("getRuleById()", () => {
    ...
    it("throws NotFoundError when rule does not exist", async () => {
      ...
      await expect(service.getRuleById("missing", tenantId)).rejects.toThrow(
        "AlertRule",
      );
    });
  });
  ...
  describe("updateRule()", () => {
    ...
    it("throws NotFoundError when rule does not exist", async () => {
      ...
      await expect(
        service.updateRule("missing", { name: "X" }, tenantId),
      ).rejects.toThrow("AlertRule");
    });
  });
  ...
  describe("deleteRule()", () => {
    it("removes a rule successfully", async () => {
      ...
      await expect(
        service.deleteRule("rule-001", tenantId),
    ...
    it("throws NotFoundError when rule does not exist", async () => {
      ...
      await expect(service.deleteRule("missing", tenantId)).rejects.toThrow(
        "AlertRule",
      );
    });
  });
  ...
  describe("getInstanceById()", () => {
    ...
    it("throws NotFoundError when instance does not exist", async () => {
      ...
      await expect(
        service.getInstanceById("missing", tenantId),
      ).rejects.toThrow("AlertInstance");
    });
  });
  ...
  describe("acknowledgeInstance()", () => {
    it("updates instance status to acknowledged", async () => {
      ...
      const mockWhereReturning = vi.fn(() => ({
        returning: vi.fn().mockResolvedValueOnce([acknowledged]),
      }));
      mockUpdateSet.mockReturnValueOnce({ where: mockWhereReturning });

      const result = await service.acknowledgeInstance(
        "instance-001",
        tenantId,
        userId,
      );
    ...
    it("throws NotFoundError when instance does not exist", async () => {
      ...
      await expect(
        service.acknowledgeInstance("missing", tenantId, userId),
      ).rejects.toThrow("AlertInstance");
    });
  });
  ...
  describe("resolveInstance()", () => {
    it("updates instance status to resolved", async () => {
      ...
      const mockWhereReturning = vi.fn(() => ({
        returning: vi.fn().mockResolvedValueOnce([resolved]),
      }));
      mockUpdateSet.mockReturnValueOnce({ where: mockWhereReturning });

      const result = await service.resolveInstance(
        "instance-001",
        tenantId,
        userId,
      );
    ...
    it("throws NotFoundError when instance does not exist", async () => {
      ...
      await expect(
        service.resolveInstance("missing", tenantId, userId),
      ).rejects.toThrow("AlertInstance");
    });
  });
  ...
  describe("updatePolicy()", () => {
    ...
    it("throws NotFoundError when policy does not exist", async () => {
      ...
      await expect(
        service.updatePolicy("missing", { name: "X" }, tenantId),
      ).rejects.toThrow("EscalationPolicy");
    });
  });
  ...
  describe("deletePolicy()", () => {
    it("removes a policy successfully", async () => {
      ...
      await expect(
        service.deletePolicy("policy-001", tenantId),
      ).resolves.not.toThrow();
      expect(mockDelete).toHaveBeenCalled();
    });
    ...
    it("throws NotFoundError when policy does not exist", async () => {
      ...
      await expect(service.deletePolicy("missing", tenantId)).rejects.toThrow(
        "EscalationPolicy",
      );
    });
  });
  ...
  describe("updateChannel()", () => {
    ...
    it("throws NotFoundError when channel does not exist", async () => {
      ...
      await expect(
        service.updateChannel("missing", { name: "X" }, tenantId),
      ).rejects.toThrow("NotificationChannel");
    });
  });
  ...
  describe("deleteChannel()", () => {
    it("removes a channel successfully", async () => {
      ...
      await expect(
        service.deleteChannel("channel-001", tenantId),
      ).resolves.not.toThrow();
      expect(mockDelete).toHaveBeenCalled();
    });
    ...
    it("throws NotFoundError when channel does not exist", async () => {
      ...
      await expect(service.deleteChannel("missing", tenantId)).rejects.toThrow(
        "NotificationChannel",
      );
    });
  });
