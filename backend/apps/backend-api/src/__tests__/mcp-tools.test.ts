...
describe("MCP Tool Registry", () => {
  ...
  // ── Registration ────────────────────────────────────────────
  it("registers all tools (>=22)", () => {
    const count = getToolCount();
    expect(count).toBeGreaterThanOrEqual(22);
  });

  it("getAllTools() returns array of tools (>=22)", () => {
    const tools = getAllTools();
    expect(tools).toSatisfy((arr: unknown[]) => arr.length >= 22);
  });
  ...
  it("getToolDescriptors() returns valid descriptors without execute function", () => {
    ...
    expect(descriptors).toSatisfy((arr: unknown[]) => arr.length >= 22);
