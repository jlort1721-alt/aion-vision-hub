...
// ─── Mock crypto.randomUUID without replacing the entire crypto object ──
vi.spyOn(crypto, "randomUUID").mockReturnValue(
  "uuid-snap-001" as `${string}-${string}-${string}-${string}-${string}`,
);
...
describe("Evidence Schemas", () => {
  ...
  describe("createEvidenceSchema", () => {
    it("accepts a valid full payload", () => {
      const valid = {
        incident_id: "11111111-1111-4111-a111-111111111111",
        device_id: "22222222-2222-4222-a222-222222222222",
        type: "snapshot",
...
describe("EvidenceService", () => {
  ...
  it("list() throws NotFoundError when incident does not belong to tenant", async () => {
    ...
    await expect(service.list(incidentId, tenantId)).rejects.toThrow(
      "Incident",
    );
  });
  ...
  it("delete() throws NotFoundError when evidence does not belong to tenant", async () => {
    ...
    await expect(service.delete("missing", tenantId)).rejects.toThrow(
      "Evidence",
    );
  });
