/**
 * Dahua Stream Monitor — 24/7 health check and auto-reconnect for REGISTRO/RTSP streams
 *
 * Run via PM2:
 *   pm2 start dist/workers/dahua-stream-monitor.js --name dahua-monitor
 *
 * Every 5 minutes:
 * 1. Queries go2rtc for all da-* streams (Dahua REGISTRO + RTSP)
 * 2. Tests each group by taking a snapshot
 * 3. If snapshot fails, re-registers the stream source in go2rtc to force reconnect
 * 4. Logs stats per cycle
 */
...
const log = {
  info: (...args: unknown[]) =>
    console.log("[dahua-monitor]", new Date().toISOString(), ...args),
  warn: (...args: unknown[]) =>
    console.warn("[dahua-monitor]", new Date().toISOString(), ...args),
  error: (...args: unknown[]) =>
    console.error("[dahua-monitor]", new Date().toISOString(), ...args),
};

const failCount = new Map<string, number>();
let cycleNum = 0;

// ── Get all Dahua streams from go2rtc ──
...
async function getDahuaStreams(): Promise<StreamInfo[]> {
  try {
    ...
    for (const [name, info] of Object.entries(data)) {
      // Only Dahua streams (da-* prefix)
      if (!name.startsWith("da-")) continue;

      const url = info.producers?.[0]?.url || "";
      if (!url) continue;

      // Group: da-sitename (without -chN suffix)
      const group = name.includes("-ch")
        ? name.substring(0, name.lastIndexOf("-ch"))
        : name;
...
async function runCheck(): Promise<void> {
  ...
  const streams = await getDahuaStreams();
  if (streams.length === 0) {
    log.warn("No Dahua streams found in go2rtc");
    return;
  }
  ...
  for (const [group, channels] of groups) {
    ...
    if (ok) {
      ...
      if (prevFails < 3) {
        ...
        log.error(
          `${group}: PERSISTENT FAILURE after 3 cycles — likely REGISTRO disconnected or network issue`,
        );
      }
    }
...
function start(): void {
  log.info(
    `Starting Dahua stream monitor (interval: ${CHECK_INTERVAL / 1000}s)`,
  );
  ...
  }, 20_000); // 20s delay (after hik-monitor's 15s)
...
function stop(): void {
  ...
  log.info("Dahua stream monitor stopped");
}
