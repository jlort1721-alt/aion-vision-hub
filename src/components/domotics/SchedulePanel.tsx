...
export default function SchedulePanel() {
  ...
  const { data: schedules = [], isLoading } = useQuery<Schedule[]>({
    ...
    queryFn: () => apiClient.get('/automation', { trigger_type: 'schedule' }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => apiClient.post('/automation', data),
    onSuccess: () => {
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({
      ...
      trigger: { type: 'schedule', config: { hour: h, minute: m, daysOfWeek: selectedDays } },
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-sm">
            <Plus className="h-4 w-4" />Nueva Programaci&oacute;n
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              onChange={(e) => setMinute(e.target.value)} className="w-20"
              placeholder="MM"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {DAYS.map((day, idx) => (
              <Button
                key={day} size="sm" variant={selectedDays.includes(idx) ? 'default' : 'outline'}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-sm">
            <Clock className="h-4 w-4" />Programaciones Activas
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground">Sin programaciones configuradas</p>
          ) : (
                ...
                <li key={s.id} className="flex items-center justify-between rounded-md border p-3">
                  <div className="space-y-1">
                    <p className="text-sm font-medium">
                      {formatTime(s.trigger.config.hour, s.trigger.config.minute)}
                    </p>
