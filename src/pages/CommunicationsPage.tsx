...
export default function CommunicationsPage() {
  ...
  useEffect(() => {
    apiClient.get<CommStats>('/twilio/stats').then(setStats).catch(() => {});
  }, []);
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">WhatsApp, llamadas y SMS via Twilio</p>
        </div>
      </div>
  ...
  return (
    <Card className="bg-slate-800/50 border-slate-700">
      ...
      <CardContent className="space-y-4">
        <div>
          <label className="text-sm text-slate-400">Número destino</label>
          <Input
          ...
          />
          <p className="text-xs text-slate-500 mt-1">Se agrega +57 automáticamente</p>
        </div>
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-md bg-slate-900 border border-slate-600 text-white p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-green-500"
          />
        </div>
        <Button
          onClick={handleSend}
      ...
      <CardContent className="space-y-4 max-w-sm mx-auto">
        ...
        <div className="text-center">
          <Input
            ...
            className="bg-slate-900 border-slate-600 text-white text-center text-xl font-mono tracking-wider"
          />
          {status !== 'idle' && (
        ...
        <div className="grid grid-cols-3 gap-2">
          {dialPadKeys.map((key) => (
            <Button
              ...
              className="h-14 text-xl font-semibold bg-slate-900 border-slate-600 text-white hover:bg-slate-700"
            >
              {key}
            </Button>
          ))}
        ...
        <div className="flex gap-3 justify-center">
          {status === 'idle' ? (
            <Button
              ...
              className="bg-green-600 hover:bg-green-700 text-white gap-2 h-14 px-8 text-lg"
            >
            ...
            <Button
              onClick={hangUp}
              className="bg-red-600 hover:bg-red-700 text-white gap-2 h-14 px-8 text-lg"
            >
          ...
          <Button
            ...
            className="border-slate-600 text-slate-400 h-14"
          >
            Borrar
          </Button>
        </div>
  ...
  return (
    <Card className="bg-slate-800/50 border-slate-700">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          <label className="text-sm text-slate-400">Mensaje (max 160 caracteres)</label>
          <textarea
            placeholder="Escribe tu mensaje..."
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            rows={3}
            maxLength={1600}
            className="w-full rounded-md bg-slate-900 border border-slate-600 text-white p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
          <p className="text-xs text-slate-500 mt-1">{message.length}/1600 caracteres</p>
        </div>
        <Button
          onClick={handleSend}
          disabled={sending || !to || !message}
          ...
          <div className="flex gap-2">
            {['', 'whatsapp', 'sms', 'voice_call'].map((f) => (
              <Button
                ...
                className={filter === f ? 'bg-blue-600' : 'border-slate-600 text-slate-400'}
              >
        ...
        {loading ? (
          ...
          <>
                  ...
                  {logs.map((log) => (
                    <tr key={log.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                      <td className="py-2 px-3 flex items-center gap-2">
                        ...
                        <span className="text-white text-xs">{log.direction === 'inbound' ? '← ' : '→ '}</span>
                      </td>
                      <td className="py-2 px-3 text-white font-mono text-xs">{log.recipient || log.sender}</td>
                      <td className="py-2 px-3 text-slate-300 max-w-[200px] truncate">{log.content}</td>
                      <td className={`py-2 px-3 font-medium ${statusColor(log.status)}`}>{log.status}</td>
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" disabled={page <= 1} onClick={() => setPage(page - 1)} className="border-slate-600 text-slate-400">Anterior</Button>
                <Button size="sm" variant="outline" disabled={page * 30 >= total} onClick={() => setPage(page + 1)} className="border-slate-600 text-slate-400">Siguiente</Button>
              </div>
            </div>
          </>
        )}
