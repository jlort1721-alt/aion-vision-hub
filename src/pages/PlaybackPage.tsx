...
function parseApiSegments(
  apiSegments: Array<{ start: string; end: string }>,
): Array<{
  start: number;
  end: number;
...
function InteractiveTimeline({
  ...
  const effectiveStart =
    viewEnd === DAY_SECONDS ? DAY_SECONDS - totalVisible : viewStart;
  const effectiveEnd = effectiveStart + totalVisible;

  const timeToPercent = (t: number) =>
    ((t - effectiveStart) / totalVisible) * 100;
  ...
  return (
    <div className="space-y-1">
        ...
        {isClipping && (
          <div className="flex items-center gap-1 ml-2">
            <Button
              ...
              onClick={onClipStartSet}
            >
              {t("playback.set_start")}{" "}
      ...
      >
        {/* Recording segments */}
        {segments.map(
          (seg: { start: number; end: number; type: string }, i: number) => {
        ...
        {events.map((evt, i) => {
          ...
          const seconds =
            evtTime.getUTCHours() * 3600 +
            evtTime.getUTCMinutes() * 60 +
            evtTime.getUTCSeconds();
        ...
        <div className="absolute bottom-0 left-0 right-0 flex justify-between px-1 text-[8px] text-muted-foreground">
          {labels
            .filter((_, i) => i % Math.max(1, Math.floor(labelCount / 9)) === 0)
            .map((t, i) => (
      ...
      <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
        <span className="flex items-center gap-1">
          <span className="w-3 h-2 bg-primary/25 rounded-sm" />{" "}
          {t("playback.continuous")}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-2 bg-warning/30 rounded-sm" />{" "}
          {t("playback.motion")}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-2 bg-destructive/30 rounded-sm" />{" "}
          {t("playback.alarm")}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-0.5 bg-destructive" />{" "}
          {t("playback.critical_event")}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-0.5 bg-warning" /> {t("playback.high")}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-0.5 bg-info" /> {t("playback.info")}
        </span>
        {clipStart !== null && (
          <span className="flex items-center gap-1">
            <span className="w-3 h-2 bg-primary/20 border border-primary rounded-sm" />{" "}
            {t("playback.clip_selection")}
          </span>
        )}
      </div>
    </div>
...
export default function PlaybackPage() {
  ...
  const [selectedDevice, setSelectedDevice] = useState(() => {
    try {
      return localStorage.getItem("aion-pb-cam") || "";
    } catch {
      return "";
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("aion-pb-cam", selectedDevice);
    } catch {
      /* */
    }
  }, [selectedDevice]);
  ...
  const device =
    cameras.find((d: any) => d.id === selectedDevice) || cameras[0];
  ...
  const { data: apiSegments } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await apiClient.post("/playback/search", {
          date: selectedDate,
          cameraId: device?.id,
        });
        const r = res as Record<string, unknown>;
        return ((r?.data as Record<string, unknown>)?.segments ??
          r?.segments ??
  ...
  const detectionEvents = useMemo(() => {
    const items =
      detectionsRaw?.data ??
      (Array.isArray(detectionsRaw) ? detectionsRaw : []);
  ...
  const createExport = useMutation({
    ...
    onSuccess: (resp) => {
      ...
      const d = (resp as Record<string, unknown>)?.data as
        | Record<string, unknown>
        | undefined;
      const sizeKB = d?.fileSize ? Math.round(Number(d.fileSize) / 1024) : 0;
      toast.success(
        d?.id
          ? `Clip exportado: ${d.filename} (${sizeKB} KB, ${d.durationSec}s)`
          : "Clip exportado exitosamente",
      );
  ...
  const skipForward = () =>
    setCurrentTime((prev) => Math.min(DAY_SECONDS, prev + 30));
  const prevSpeed = () =>
    setSpeed((prev) => SPEEDS[Math.max(0, SPEEDS.indexOf(prev) - 1)]);
  const nextSpeed = () =>
    setSpeed(
      (prev) => SPEEDS[Math.min(SPEEDS.length - 1, SPEEDS.indexOf(prev) + 1)],
    );
  ...
  const handleSnapshot = () => {
    ...
    if (video && video.readyState >= 2) {
      ...
      a.click();
      toast.success(
        `Captura guardada: ${device?.name} a las ${formatTime(currentTime)}`,
      );
    } else {
      toast.info(
        `Captura: ${device?.name || "cámara"} a las ${formatTime(currentTime)} (sin video activo)`,
      );
    }
  };
  ...
  const startClipping = () => {
    ...
    setClipEnd(null);
    toast.info(
      'Click "Set Start" and "Set End" on the timeline, or use current playhead position',
    );
  };
  ...
  const isInRecording = segments.some(
            ...
            {isLoading ? (
              ...
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue placeholder={t("playback.select_camera")} />
                </SelectTrigger>
                <SelectContent>
                  {cameras.map((d: any) => (
            ...
            {deviceEvents.length === 0 ? (
              ...
              deviceEvents
                ...
                .map((evt: any) => {
                  ...
                  const evtSeconds =
                    evtDate.getUTCHours() * 3600 +
                    evtDate.getUTCMinutes() * 60 +
                    evtDate.getUTCSeconds();
                  return (
                    <div
                      key={evt.id}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-medium truncate">
                          {evt.title || evt.description || evt.event_type}
                        </p>
                        <p className="text-[9px] text-muted-foreground">
                          {formatTime(evtSeconds)} •{" "}
        ...
        <div className="p-2 border-t">
          ...
          <ScrollArea className="max-h-32">
            {playbackRequests.slice(0, 5).map((req: any) => (
              <div
                key={req.id}
                className="text-[10px] p-1.5 rounded hover:bg-muted/50"
              >
          ...
          {!device ? (
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t("playback.select_camera_to_play")}
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-medium text-muted-foreground">
                {device.name} — Canal {selectedChannel}
              </p>
              <p className="text-[11px] text-muted-foreground/60 mt-1">
                {selectedDate} • {formatTime(currentTime)}
              </p>
              <Badge
                variant={isInRecording ? "default" : "secondary"}
                className="text-[10px] mt-2"
              >
                {isInRecording
                  ? t("playback.recording_available")
                  : t("playback.no_recording")}
              </Badge>
              {videoError && (
                <p className="text-[10px] text-destructive/60 mt-2">
                  {t("playback.stream_unavailable")}
                </p>
              )}
            </div>
          )}
          ...
          {device && (
            <>
              <div className="absolute top-3 left-3 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <Badge
                  variant="outline"
                  className="text-[9px] bg-black/60 backdrop-blur text-white border-white/20"
                >
                  {device.brand} • {device.model}
                </Badge>
                <Badge
                  variant="outline"
                  className="text-[9px] bg-black/60 backdrop-blur text-white border-white/20 font-mono"
                >
                  CH{selectedChannel}
                </Badge>
                {playing && (
                  <Badge
                    variant="outline"
                    className="text-[9px] bg-red-600/80 backdrop-blur text-white border-red-400/40 font-mono"
                  >
                    ● {speed}×
                  </Badge>
                )}
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-1">
              <Button
                ...
                className="h-7 text-xs"
                onClick={() =>
                  isClipping ? setIsClipping(false) : startClipping()
                }
              >
                <Scissors className="mr-1 h-3 w-3" />{" "}
                {isClipping ? t("playback.cancel") : t("playback.clip")}
              </Button>
              <Button
                variant="outline"
                size="sm"
              ...
              >
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>{t("playback.export_clip_title")}</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    <div className="p-3 rounded-md bg-muted text-xs space-y-1">
                      <p>
                        <strong>{t("playback.export_camera")}:</strong>{" "}
                        {device?.name}
                      </p>
                      <p>
                        <strong>{t("playback.export_channel")}:</strong>{" "}
                        {selectedChannel}
                      </p>
                      <p>
                        <strong>{t("playback.export_date")}:</strong>{" "}
                        {selectedDate}
                      </p>
                      <p>
                        <strong>{t("playback.export_start")}:</strong>{" "}
                        {clipStart !== null ? formatTime(clipStart) : "—"}
                      </p>
                      <p>
                        <strong>{t("playback.export_end")}:</strong>{" "}
                        {clipEnd !== null ? formatTime(clipEnd) : "—"}
                      </p>
                      <p>
                        <strong>{t("playback.export_duration")}:</strong>{" "}
                        {clipStart !== null && clipEnd !== null
