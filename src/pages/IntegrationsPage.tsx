...
import type { ApiIntegration, ApiMcpConnector } from "@/types/api-entities";
...
export default function IntegrationsPage() {
  ...
  const integrations = rawIntegrations as ApiIntegration[];
  const connectors = rawConnectors as ApiMcpConnector[];
  ...
  const handleIntegrationTest = async (id: string) => {
    ...
    try {
      ...
      toast.success(
        `Prueba: ${result?.message || "OK"} (${result?.latency_ms || "?"}ms)`,
      );
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Error en prueba");
    } finally {
  ...
  const handleIntegrationToggle = async (id: string) => {
    ...
    try {
      const updated: any = await apiClient.patch(`/integrations/${id}`, {
        action: "toggle",
      });
      toast.success(
        `Integración ${updated?.status === "active" ? "activada" : "desactivada"}`,
      );
  ...
  const handleMcpHealthCheck = async (id: string) => {
    ...
    try {
      ...
      toast.success(
        `Salud: ${result?.health || "OK"} (${result?.check_latency_ms || "?"}ms)`,
      );
  ...
  const handleMcpToggle = async (id: string) => {
    ...
    try {
      const updated: any = await apiClient.patch(`/mcp/connectors/${id}`, {
        action: "toggle",
      });
      toast.success(
        `Conector ${updated?.status === "connected" ? "conectado" : "desconectado"}`,
      );
      queryClient.invalidateQueries({ queryKey: ["mcp_connectors"] });
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Error");
  ...
  if (hasError)
    return (
      <ErrorState
        error={(intError || connError) as Error}
        onRetry={() => {
          refetchInt();
          refetchConn();
        }}
      ...
      <Tabs defaultValue="active">
        <TabsList>
          <TabsTrigger value="active">
            {t("integrations.active")} ({integrations.length})
          </TabsTrigger>
          <TabsTrigger value="mcp">
            {t("integrations.mcp_connectors")} ({connectors.length})
          </TabsTrigger>
          <TabsTrigger value="catalog">
            {t("integrations.catalog")} ({MCP_CONNECTOR_CATALOG.length})
          </TabsTrigger>
        </TabsList>
              ...
              <CardContent>
                ...
                {(n8nStatus as any)?.data?.url && (
                  <p className="text-xs text-muted-foreground mt-2">
                    URL:{" "}
                    {(n8nStatus as any).data.url.replace(/^https?:\/\//, "***")}
                  </p>
                )}
              </CardContent>
            </Card>
                ...
                <Card key={connector.id}>
                  ...
                  <CardContent className="space-y-2">
                    <div className="flex flex-wrap gap-1">
                      {((connector.scopes as string[]) || []).map(
                        (s: string) => (
                          <Badge
                            key={s}
                            variant="outline"
                    ...
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                      <span>
                        {t("integrations.errors")}: {connector.error_count} •{" "}
                        {t("integrations.health")}: {connector.health}
                      </span>
                      {connector.last_check && (
                        <span>
                          {new Date(connector.last_check).toLocaleTimeString(
                            "es-CO",
                          )}
                        </span>
                      )}
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        size="sm"
                        className="h-7 text-xs flex-1"
                        onClick={() => handleMcpHealthCheck(connector.id)}
                        disabled={
                          actionLoading === `mcp-health-${connector.id}`
                        }
                      >
                        {actionLoading === `mcp-health-${connector.id}` ? (
                          <Loader2 className="mr-1 h-3 w-3 animate-spin" />
                        ...
                        {t("integrations.health_check")}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        className="h-7 text-xs flex-1"
                        onClick={() => handleMcpToggle(connector.id)}
                        disabled={
                          actionLoading === `mcp-toggle-${connector.id}`
                        }
                      >
                        {actionLoading === `mcp-toggle-${connector.id}` ? (
                          <Loader2 className="mr-1 h-3 w-3 animate-spin" />
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" className="h-7 text-xs">
                      <Plus className="mr-1 h-3 w-3" /> {t("common.connect")}
                    </Button>
                  </div>
                  <div className="mt-2 flex flex-wrap gap-1">
                    {(def.availableTools || []).map((tool) => (
                      <Badge
                        key={tool.name}
                        variant="secondary"
                        className="text-[8px]"
