...
const statusLabels: Record<string, string> = {
  open: 'Abierto', investigating: 'Investigando', in_progress: 'En progreso',
  resolved: 'Resuelto', closed: 'Cerrado',
};
const statusBadgeVariant = (s: string) =>
  s === 'open' ? 'destructive' as const :
  s === 'resolved' || s === 'closed' ? 'secondary' as const : 'outline' as const;
...
export default function IncidentsPage() {
  ...
  const selectedInc = selected ? incidents.find((i: any) => i.id === selected) : null;

  // Stats
  const openCount = incidents.filter((i: any) => i.status === 'open').length;
  const investigatingCount = incidents.filter((i: any) => i.status === 'investigating' || i.status === 'in_progress').length;
  ...
  const handleCreate = async () => {
    if (!(newIncident.title || '').trim()) { toast.error('El título es requerido'); return; }
    ...
    } catch (err) { toast.error(err instanceof Error ? err.message : 'Error al crear'); } finally { setActionLoading(null); }
  };
  ...
  const handleComment = async () => {
    ...
    try {
      await apiClient.post(`/incidents/${selectedInc.id}/comments`, { content: comment });
      toast.success('Comentario agregado');
      setComment('');
      queryClient.invalidateQueries({ queryKey: ['incidents'] });
    } catch (err) { toast.error(err instanceof Error ? err.message : 'Error'); } finally { setActionLoading(null); }
  };
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      await apiClient.patch(`/incidents/${selectedInc.id}`, { status: newStatus });
  ...
  const handleAiSummary = async () => {
    ...
    try {
      await apiClient.post('/ai/chat', {
        messages: [{ role: 'user', content: `Analiza este incidente de seguridad y genera un resumen ejecutivo: "${selectedInc.title}" - ${selectedInc.description || 'Sin descripción'}. Prioridad: ${selectedInc.priority}. Estado: ${selectedInc.status}.` }],
      });
  ...
  return (
    <PageShell
      ...
      actions={
        <>
        <EvidenceExport incidents={incidents} />
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="mr-1 h-3 w-3" /> Nuevo</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Crear Incidente</DialogTitle></DialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <Label>Título</Label>
                <Input value={newIncident.title} onChange={e => setNewIncident(p => ({ ...p, title: e.target.value }))} placeholder="Título del incidente" />
              </div>
              <div className="space-y-2">
                <Label>Descripción</Label>
                <Textarea value={newIncident.description} onChange={e => setNewIncident(p => ({ ...p, description: e.target.value }))} placeholder="Descripción detallada" />
              </div>
    ...
    <div className="flex flex-col md:flex-row h-full">
      ...
      <div className={cn("w-full md:w-80 border-r flex flex-col", selected && "hidden md:flex")}>
        ...
        <div className="px-3 py-2 border-b flex items-center gap-2 text-xs">
          <Badge variant="destructive" className="text-[10px]">{openCount} abiertos</Badge>
          <Badge variant="outline" className="text-[10px]">{investigatingCount} en curso</Badge>
          <Badge variant="secondary" className="text-[10px]">{resolvedCount} resueltos</Badge>
        </div>
        ...
        <div className="px-3 py-2 border-b space-y-2">
          <div className="relative">
            ...
            <Input placeholder="Buscar incidentes..." className="pl-7 h-7 text-xs" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
          <div className="flex items-center gap-1">
            <Filter className="h-3 w-3 text-muted-foreground" />
            {['all', 'open', 'investigating', 'resolved', 'closed'].map(s => (
              <Button key={s} variant={statusFilter === s ? 'default' : 'ghost'} size="sm" className="h-6 text-[10px] px-2" onClick={() => setStatusFilter(s)}>
      ...
      {selectedInc ? (
        <div className="flex-1 overflow-auto p-4 sm:p-6 space-y-4">
          <button onClick={() => setSelected(null)} className="md:hidden text-xs text-muted-foreground mb-2 flex items-center gap-1 hover:text-foreground">&larr; Volver</button>
          <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <Badge variant={statusBadgeVariant(selectedInc.status)}>{statusLabels[selectedInc.status] || selectedInc.status}</Badge>
            ...
            <div className="flex gap-2 flex-wrap">
              <Button variant="outline" size="sm" onClick={handleAiSummary} disabled={!!actionLoading}>
                {actionLoading === 'ai' ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <Bot className="mr-1 h-3 w-3" />} Resumen IA
              </Button>
              {selectedInc.status !== 'closed' && selectedInc.status !== 'resolved' && (
                <>
                  <Button variant="secondary" size="sm" onClick={() => handleStatusChange('resolved')} disabled={!!actionLoading}>
                    {actionLoading === 'resolved' ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <CheckCircle2 className="mr-1 h-3 w-3" />} Resolver
                  </Button>
                  <Button variant="destructive" size="sm" onClick={() => handleStatusChange('closed')} disabled={!!actionLoading}>
                    {actionLoading === 'closed' ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <XCircle className="mr-1 h-3 w-3" />} Cerrar
                  </Button>
                </>
              )}
            </div>
          ...
          {selectedInc.ai_summary && (
            <Card className="border-primary/30 bg-primary/5">
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-1"><Bot className="h-3 w-3" /> Resumen IA</CardTitle></CardHeader>
              ...
              <Card>
                <CardContent className="space-y-3 pt-4">
                  ...
                  {selectedInc.status !== 'closed' && (
                    <>
                      <Textarea placeholder="Agregar comentario..." className="text-sm min-h-[60px]" value={comment} onChange={e => setComment(e.target.value)} />
                      <div className="flex gap-2">
                        <Button size="sm" onClick={handleComment} disabled={!comment.trim() || actionLoading === 'comment'}>
                          {actionLoading === 'comment' ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <MessageSquare className="mr-1 h-3 w-3" />} Comentar
                        </Button>
                      </div>
                    </>
                  )}
