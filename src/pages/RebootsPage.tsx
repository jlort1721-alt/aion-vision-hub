...
const REBOOT_PROCEDURES = [
  {
    id: "proc-001",
    title: "Reinicio remoto de cámara IP",
    steps: [
      "Verificar conectividad de red",
      "Intentar ping al dispositivo",
      "Enviar comando de reinicio ONVIF/ISAPI",
      "Esperar 60 segundos",
      "Verificar stream RTSP",
      "Confirmar resolución o escalar",
    ],
    difficulty: "basico",
  },
  {
    id: "proc-002",
    title: "Reinicio de NVR",
    steps: [
      "Notificar al supervisor",
      "Verificar que no hay grabaciones activas críticas",
      "Enviar comando de reinicio",
      "Esperar 2-3 minutos",
      "Verificar canales reconectados",
      "Confirmar grabación reanudada",
    ],
    difficulty: "intermedio",
  },
  {
    id: "proc-003",
    title: "Reinicio de switch/PoE",
    steps: [
      "Identificar dispositivos afectados",
      "Notificar a central",
      "Reiniciar puerto PoE específico",
      "Si no resuelve, reiniciar switch completo",
      "Esperar reconexión de todos los dispositivos",
  ...
  {
    id: "proc-004",
    title: "Verificación post-reinicio",
    steps: [
      "Confirmar IP accesible",
      "Verificar stream de video",
      "Verificar grabación local/NVR",
      "Verificar eventos inteligentes",
...
export default function RebootsPage() {
  ...
  const completeMutation = useMutation({
    mutationFn: async ({
      ...
      const resp = await rebootsApi.complete(id, {
        status,
        result,
        recoveryTimeSeconds,
      });
  ...
  const procedure = selectedProcedure
    ? REBOOT_PROCEDURES.find((p) => p.id === selectedProcedure)
    : null;
  const successCount = rebootTasks.filter(
    (t: any) => t.status === "completed",
  ).length;
  const failedCount = rebootTasks.filter(
  ...
  const handleAdd = () => {
    ...
    createMutation.mutate({
      deviceId: form.device_id || undefined,
      reason: form.reason,
    });
    setAddOpen(false);
        ...
        [
          {
            role: "system",
            content:
              "Eres un experto en diagnostico de sistemas de videovigilancia y seguridad electronica. Responde en espanol, de forma concisa y tecnica.",
          },
          {
            role: "user",
            content: `Diagnostica el siguiente escenario para el procedimiento "${procedure.title}":\nPasos del procedimiento: ${(procedure.steps || []).join(" -> ")}${offlineInfo}\n\nProporciona un diagnostico breve con posibles causas y estado estimado.`,
          },
        ],
        ...
        [
          {
            role: "system",
            content:
              "Eres un experto en resolucion de problemas de sistemas de videovigilancia y seguridad electronica. Responde en espanol con pasos concretos y accionables.",
          },
          {
            role: "user",
            content: `Sugiere una solucion para el procedimiento "${procedure.title}" (nivel: ${procedure.difficulty}).${offlineInfo}\n\nProporciona pasos especificos para resolver el problema, incluyendo comandos o acciones concretas si aplica.`,
          },
        ],
        { temperature: 0.4, maxTokens: 1024 },
      );
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)]">
        ...
        <div className="px-4 py-3 border-b space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => setAddOpen(true)}>
              <Plus className="mr-1 h-3 w-3" /> {t("reboots.initiate")}
            </Button>
          </div>
          <div className="grid grid-cols-4 gap-2">
            <Card className="p-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">
                    {t("reboots.devices_needing_attention")}
                  </p>
                  <p className="text-lg font-bold">{offlineDevices.length}</p>
                </div>
              </div>
            </Card>
            <Card className="p-2">
            ...
            <Card className="p-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">
                    {t("reboots.total_reboots")}
                  </p>
                  <p className="text-lg font-bold">{rebootTasks.length}</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            <div className="px-4 py-2 border-b space-y-2">
              ...
              <div className="flex items-center gap-1">
                {["all", "pending", "completed", "failed"].map((s) => (
                  <Button
                    key={s}
                    variant={taskStatusFilter === s ? "default" : "ghost"}
                    size="sm"
                  ...
                  >
                    ...
                    {s !== "all" && (
                      ...
                      >
                        {rebootTasks.filter((t: any) => t.status === s).length}
                      </Badge>
                    )}
                  </Button>
                ))}
                      ...
                      .map((task: any) => (
                        <TableRow key={task.id}>
                          ...
                          <TableCell className="text-xs font-mono">
                            {task.recovery_time_seconds
                              ? `${task.recovery_time_seconds}s`
                              : task.recoveryTimeSeconds
                                ? `${task.recoveryTimeSeconds}s`
                                : "\u2014"}
                          </TableCell>
                          <TableCell className="text-xs font-mono">
                            {new Date(
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      completeMutation.mutate({
                                        id: task.id,
                                        status: "failed",
                                        result: "Dispositivo inalcanzable",
                                      })
                                    }
                                  >
              ...
              {REBOOT_PROCEDURES.map((proc) => (
                <Card
                  ...
                  className={cn(
                    ...
                    selectedProcedure === proc.id && "border-primary",
                  )}
                  onClick={() => {
                  ...
                  <CardContent className="p-3">
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant="outline" className="text-[9px]">
                        {difficultyLabels[proc.difficulty] || proc.difficulty}
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center gap-2 py-2">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        {aiLoading === "diagnose"
                          ...
                          : "Generando sugerencia de solución..."}
                      </span>
                    </div>
                  ) : aiResult ? (
                    <p className="text-[10px] text-muted-foreground whitespace-pre-wrap max-h-40 overflow-auto">
                      {aiResult}
                    </p>
                  ) : (
                    <p className="text-[10px] text-muted-foreground">
                      {procedure.difficulty === "basico" ||
                      procedure.difficulty === "basic"
                        ...
                        : procedure.difficulty === "intermedio" ||
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("reboots.initiate")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => setForm((p) => ({ ...p, device_id: v }))}
              >
                <SelectTrigger>
